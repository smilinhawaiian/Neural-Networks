...
class Perceptron:
    # constructor here
    def __init__(self, input_size, my_learning_rate, my_label):   # first parameter of any class method is self
...
class TypeClassifier:
    def __init__(self, input_size, c_learning_rate, num_outputs):
        self.perceptron_list = []  # create a list of perceptrons with their labels
        for index in range(num_outputs):
            # create perceptron labels using index
            self.perceptron_list.append(Perceptron(input_size, c_learning_rate, index))
...
if __name__ == "__main__":
    ...
    digit_classifier = TypeClassifier(num_inputs, learning_rate, num_classes)
    ...
    plt.title('Confusion_Matrix_MNIST_training_step={0}_Epochs={1}of{2}'.format(learning_rate, curr_epoch, epochs))
    plt.savefig(('Confusion_Matrix_Step' + str(learning_rate) + '_Epochs' + str(curr_epoch) + 'of' + str(epochs)
    ...
    plt.legend(loc=2)
    plt.axis('tight')
    plt.title('Accuracy_during_MNIST_training_step={0}_Epochs={1}of{2}'.format(learning_rate, curr_epoch, epochs))
    plt.savefig(('Perceptron_Accuracy_Step' + str(learning_rate) + '_Epochs' + str(curr_epoch) + 'of' + str(epochs)
                 + '_Figure_' + str(plot_num) + '.jpg'), bbox_inches='tight')
